...
## Set up environment
Let's set up the environment where we work.Let's assume you have a folder on your desktop called `achems`, and we will use that as our working directory. We will download our input file (the count table) there.

If a library hasn't been installed yet, you can install it using e.g. `install.packages("ggplot2")`. 
...
Let's cluster the samples based on OR expression. Well, they actually cluster by litter... So it's important to model that as we did before!
...
```{r pca}
or_pca <- data.frame(prcomp(t(log1p(orCountData)))$x)
